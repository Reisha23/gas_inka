...
def register_user(nama_user, password_user):
    hashed_password = hashlib.md5(password_user.encode()).hexdigest()
    query = '''
        INSERT INTO pengguna (nama_user, password_user) 
        VALUES (%s, %s)
    '''
    ...
    if conn:
        cursor = conn.cursor()
        try:
            cursor.execute(query, (nama_user, hashed_password))
            conn.commit()
...
def register_page():
    ...
    if st.button("Register"):
        if password_user == confirm_password:
            register_user(nama_user, password_user)
        else:
